...
export async function renderWorksheetPdf(
  ...
  const browser = await puppeteer.launch({
    headless: true,
    // Use an apt-installed system Chromium instead of Puppeteer's bundled
    // download — the bundled Chrome-for-Testing binary on Linux ARM is a
    // shell wrapper script with bash-specific syntax that fails under
    // Debian's default dash shell. A real apt package avoids that entirely.
